...
function resize(){
  ...
  // Set responsive uCenterOffset to shift the black hole
  if (w <= 768) {
    // Mobile: shift black hole to the top
    uniforms.uCenterOffset.value.set(0.0, 0.22);
  } else if (w <= 1024) {
    // Tablet: 60/40 layout, shift black hole to the right
    uniforms.uCenterOffset.value.set(0.18, 0.0);
  } else {
    // Desktop: shift black hole to the right
    uniforms.uCenterOffset.value.set(0.24, 0.0);
  }
...
function triggerEntranceAnimations() {
  ...
  if (typeof gsap === 'undefined') {
    console.warn('[AegisAI] GSAP not loaded. Skipping animations.');
    return;
  }
  
  // Set initial states for elements
  gsap.set('#aegis-nav', { y: -50, opacity: 0 });
  ...
  gsap.set('.hero-btn-wrap .btn', { scale: 0.8, opacity: 0 });
  gsap.set('.scroll-indicator', { y: -20, opacity: 0 });

  const tl = gsap.timeline({ defaults: { ease: 'power3.out', duration: 1.0 } });

  tl.to('#aegis-nav', { y: 0, opacity: 1, duration: 0.8 })
    .to('.hero-badge', { y: 0, opacity: 1, duration: 0.6 }, '-=0.4')
    .to('.hero-title-line', { y: 0, opacity: 1, stagger: 0.15, duration: 0.8 }, '-=0.3')
    .to('.hero-subtitle', { y: 0, opacity: 1, duration: 0.7 }, '-=0.6')
    .to('.hero-desc', { y: 0, opacity: 1, duration: 0.8 }, '-=0.5')
    .to('.hero-btn-wrap .btn', { scale: 1, opacity: 1, stagger: 0.1, ease: 'back.out(1.5)', duration: 0.6 }, '-=0.6')
    .to('.scroll-indicator', { y: 0, opacity: 0.6, duration: 0.8 }, '-=0.4');
}
...
function frame(){
  ...
  // Save base camera position and target orientation
  ...
  // 1. Zoom toward the black hole: zoom in up to 60% of the original distance.
  const scrollFactor = 1.0 - scrollProgress * 0.60;
  camera.position.multiplyScalar(scrollFactor);

  // 2. Adjust bloom intensity baseline and chromatic aberration based on scroll progress
  if (bloomPass) {
    bloomPass.strength = S.params.bloomStrength + scrollProgress * 0.85;
  }
  if (compositePass) {
    compositePass.uniforms.uCA.value = S.params.ca + scrollProgress * 0.0055;
  }
  ...
  // Calculate camera local coordinate axes to apply parallax in screen space
  const right = new THREE.Vector3();
  const up = new THREE.Vector3();
  camera.matrixWorld.extractBasis(right, up, new THREE.Vector3());
  
  // Subtle parallax translation: offset camera position based on mouse coordinate
  const dist = camera.position.length();
  const parallaxScale = dist * 0.04; 
  camera.position.addScaledVector(right, mouseX * parallaxScale);
  camera.position.addScaledVector(up, -mouseY * parallaxScale);
  camera.lookAt(0, 0, 0);
  ...
  // Restore base camera position so OrbitControls and flight systems function correctly
...
function bindUI(){
  ...
  // AegisAI specific UI bindings
  const navbar = $('aegis-nav');
  ...
  if (diagBtn && diagPanel) {
    diagBtn.addEventListener('click', (e) => {
      e.preventDefault();
      const isHidden = diagPanel.classList.toggle('expert-hidden');
      diagBtn.textContent = isHidden ? '⚙ SHOW DIAGNOSTICS' : '✕ HIDE DIAGNOSTICS';
      notify(isHidden ? 'DIAGNOSTICS PANEL HIDDEN' : 'DIAGNOSTICS PANEL OPENED');
    });
  }

  // Mobile menu toggle
  const burger = $('aegis-burger');
  const navMenu = $('aegis-nav-menu');
  if (burger && navMenu) {
  ...
  document.querySelectorAll('.aegis-nav-link').forEach(link => {
  ...
  if (cursor) {
    ...
    document.querySelectorAll('a, button, [role="button"], input[type="range"], .btn').forEach(el => {
...
const agentDescriptions = {
  planning: {
    ...
    desc: 'Decomposes complex requests, plans logic execution, and coordinates sub-agent execution loops.'
  },
  knowledge: {
    title: 'Knowledge Retrieval Agent',
    desc: 'Queries operational logs, vector documentation database, and contextualizes findings via RAG.'
  },
  operations: {
    title: 'Operations Agent',
    desc: 'Monitors real-time SCADA sensor streams and controls process loops within safety envelopes.'
  },
  maintenance: {
    title: 'Maintenance Agent',
    desc: 'Generates detailed preventative work orders, schedules repairs, and writes diagnostic reports.'
  },
  analytics: {
    title: 'Analytics Agent',
    desc: 'Processes historical trends, creates forecasts, and visualizes system state changes.'
  },
  risk: {
    title: 'Risk Agent',
    desc: 'Validates agent actions against enterprise safety bounds and runs proactive safety models.'
  }
};

function initScrollExperience() {
  if (typeof Lenis === 'undefined' || typeof gsap === 'undefined' || typeof ScrollTrigger === 'undefined') {
    console.warn('[AegisAI] Scroll libraries not loaded.');
    return;
  }
  ...
  // Track active navbar links on scroll
  ...
  document.querySelectorAll('.glass-section').forEach(sec => {
    ...
    const paragraph = sec.querySelector('.section-paragraph');
    const diagram = sec.querySelector('.diagram-container');
    const orbit = sec.querySelector('.agent-orbit-container, .tech-orbit-container');
    const timeline = sec.querySelector('.roadmap-container');
    ...
    if (kicker) tl.from(kicker, { opacity: 0, y: 15, duration: 0.6 });
    if (heading) tl.from(heading, { opacity: 0, y: 25, duration: 0.7, ease: 'power3.out' }, '-=0.4');
    if (paragraph) tl.from(paragraph, { opacity: 0, y: 20, duration: 0.7 }, '-=0.5');
    if (diagram) tl.from(diagram, { opacity: 0, scale: 0.95, duration: 0.8 }, '-=0.4');
    if (orbit) tl.from(orbit, { opacity: 0, scale: 0.9, duration: 0.8 }, '-=0.5');
    if (timeline) tl.from(timeline, { opacity: 0, duration: 0.8 }, '-=0.4');
  ...
  // Section 2: Problem Staggered Appear & Connection SVG lines
  animateProblemConnections();

  // Section 3: Solution Flow diagram particles and nodes
  animateSolutionDiagram();

  // Section 4: Orbiting Agents
  initAgentOrbit();

  // Section 6: Tech stack Orbit
  initTechStackOrbit();

  // Section 8: KPI Countups
  initKPICounters();

  // Section 9: Future Vision Node Project
  initFutureVision();


  // Section 11: Timeline Highlight
  initTimelineHighlight();
}

function animateProblemConnections() {
  const svg = document.getElementById('problem-svg');
  const container = document.querySelector('.problem-layout');
  if (!svg || !container) return;

  const cards = [
    document.getElementById('problem-1'),
    document.getElementById('problem-2'),
    document.getElementById('problem-3'),
    document.getElementById('problem-4'),
    document.getElementById('problem-5')
  ];
  ...
  ScrollTrigger.create({
    trigger: '#problem',
    start: 'top 50%',
    onEnter: () => setTimeout(drawPaths, 200),
  });
}

function animateSolutionDiagram() {
  const svg = document.getElementById('solution-diagram-svg');
  ...
  function drawDiagramLines() {
    ...
    for (let i = 1; i < points.length; i++) {
      ...
      const pathBg = document.createElementNS('http://www.w3.org/2000/svg', 'path');
      pathBg.setAttribute('d', `M ${p0.x} ${p0.y} L ${p1.x} ${p1.y}`);
      svg.appendChild(pathBg);

      const pathActive = document.createElementNS('http://www.w3.org/2000/svg', 'path');
      pathActive.setAttribute('d', `M ${p0.x} ${p0.y} L ${p1.x} ${p1.y}`);
      pathActive.classList.add('active');
      svg.appendChild(pathActive);
    }
  }

  window.addEventListener('resize', drawDiagramLines);
  ScrollTrigger.create({
    trigger: '#architecture',
    start: 'top 50%',
    onEnter: () => setTimeout(drawDiagramLines, 200),
  });
}

function initAgentOrbit() {
  const container = document.getElementById('agent-orbit-container');
  const svg = document.getElementById('agent-orbit-svg');
  const circles = container.querySelectorAll('.agent-circle');
  ...
  // Setup circular wiring connections
  function drawOrbitRings() {
    ...
    ring.setAttribute('stroke', 'rgba(245, 158, 11, 0.15)');
  ...
  // Interaction for clicking agents
  circles.forEach(circle => {
    circle.addEventListener('click', () => {
      const type = circle.getAttribute('data-agent');
      const data = agentDescriptions[type];
      if (!data) return;

      // Slick fade text swap animation using GSAP
      gsap.fromTo('#agent-info-card', 
        { opacity: 0.5, y: 5 }, 
        { opacity: 1, y: 0, duration: 0.4, onStart: () => {
            document.getElementById('agent-card-title').textContent = data.title;
            document.getElementById('agent-card-desc').textContent = data.desc;
          }
        }
      );
    });
...
function initTechStackOrbit() {
  const container = document.getElementById('tech-orbit-container');
  const svg = document.getElementById('tech-orbit-svg');
  const icons = container.querySelectorAll('.tech-icon');
  if (!container || icons.length === 0) return;

  function drawTechRings() {
    svg.innerHTML = '';
    const isMobile = window.innerWidth <= 768;
    const cx = container.offsetWidth / 2;
    const cy = container.offsetHeight / 2;
    ...
    [rInner, rOuter].forEach(r => {
      ...
      ring.setAttribute('fill', 'none');
      ring.setAttribute('stroke', 'rgba(245, 158, 11, 0.1)');
      ring.setAttribute('stroke-width', '1');
      svg.appendChild(ring);
    });
  }

  window.addEventListener('resize', drawTechRings);
  drawTechRings();
}

function initKPICounters() {
  const kpis = document.querySelectorAll('.metric-val');
  kpis.forEach(kpi => {
    ...
    ScrollTrigger.create({
      ...
      onEnter: () => {
        ...
        gsap.to(val, {
          ...
          ease: 'power2.out',
          onUpdate: () => {
            kpi.textContent = Math.floor(val.value) + '%';
          }
        });
      }
    });
...
function initFutureVision() {
  const nodes = document.querySelectorAll('.future-node');
  const container = document.querySelector('.future-vision-layout');
  ...
  window.addEventListener('resize', layoutFutureNodes);
  layoutFutureNodes();
...
function initTimelineHighlight() {
  ...
  if (milestones.length === 0 || !progress) return;
  ...
  ScrollTrigger.create({
    ...
    onUpdate: (self) => {
      ...
      milestones.forEach((stone) => {
        const index = parseInt(stone.getAttribute('data-index'));
        const triggerPct = ((index - 1) / (milestones.length - 1)) * 100;
        if (percentage >= triggerPct - 5) {
